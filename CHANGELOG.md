...
5. 健康分改为读取真实 `outputs/.ingest-tracker.md` 和最新维护报告；外部 Claude Code 已完成的体检/消化会被识别，不再只依赖插件自己的 `/check` 历史。
6. 设置页新增知识库指南“检查并修复”，可在规则文件缺失时创建默认 `AGENTS.md`，或补齐用户指定 Markdown 的最小运行规则。
7. 知识库状态栏拆分“知识库健康”和“体检新鲜度”两条进度条；体检新鲜度会随未体检天数下降并显示绿/黄/红状态点，但不拉低知识库健康分。
8. 健康概览右侧状态统一为两字短标签：健康/风险/异常，新鲜/待检/过期/无检，避免进度条末端错位。
9. 普通会话新增工作区选择器；未选择文件夹前不能发送，附加笔记只作为上下文，不再默认把当前 Vault 当工作区。
