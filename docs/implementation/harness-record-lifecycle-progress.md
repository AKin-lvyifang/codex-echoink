...
恢复证据和可重放 Journal，但尚未把 immutable payload 字节移入 Trash，也未接入
生产启动恢复或自动维护。全量 V1→V2 reader cutover、Raw quarantine/retirement
和真实迁移仍未完成。
...
| Phase 2：数据治理 | 进行中 | `b2db2f5` 至 `48264d6` 的治理 checkpoint；live clear/delete、History V2、metadata-only Raw GC preview 与 Run Record 只读 retention preview/逻辑过期已提交 | payload 物理 retirement、Journal 启动恢复、Raw quarantine/二次扫描、migration validator 与 V2→V1 exporter |
...
- Archive Catalog 与设置文案已按 retention 合同改为“仅保留期内可查”，并区分
  `expired`、`missing` 与 `corrupt`；生产 reader、自动维护和启动恢复尚未因此
  获得执行授权。
...
- Run Record retention 已建立 30 天 payload、90 天 summary 的双次稳定只读
  preview、恢复证据 authority、不可变 plan、append-only progress Journal 与
  payload → Attempt summary → Workflow summary 的逻辑 tombstone 顺序。plan
  只输出 metadata；底层严格 inventory 会读取并校验完整 payload events，但不会
  把正文写入 plan 或产品输出。当前 checkpoint 只退休读取 authority：immutable
  payload 字节尚未通过 Trash 物理退休，retention Journal 尚未进入 Storage
  Inventory、生产启动恢复或自动维护调度。
...
1. 为 expired payload generation 接入可恢复 Trash retirement，并把 retention
   Journal root 纳入 Storage Inventory 与生产启动恢复。
2. 接入 Raw quarantine 与至少 7 天后的第二次完整稳定 owner scan，再完成
   migration validator 与 V2 → V1 exporter。
