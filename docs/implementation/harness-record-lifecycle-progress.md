...
旧数据产生时还没有正式 Run Record Store，因此不能事后伪造新的 Workflow Run 和
Native Execution。现在新增独立的只读兼容档案：它只保存 526 条旧关系的匿名摘要，
同时绑定迁移投影指纹和原始 V1 快照摘要。迁移时必须从该档案严格回读；摘要、原始
V1 或 owner edge 任一变化都会继续阻断。原始 V1 Conversation 全部保留，不覆盖、
不删除。

正式 Run Record Store 会在切换前建立完整的空目录结构。历史关系由兼容档案负责，
上线后的新运行才写入正式 Store，二者不会互相伪装。已存在的 Settings 和 Native
Store 仍按原有严格规则回读。

真实 V1 的三类已知兼容差异已经按固定规则收口：

- 会话标题只清除首尾空格。
- 旧工具标题超过 V2 上限时保留前 512 个字符，完整旧值仍在 V1 原件。
- 旧 Snapshot 的 `sourceMessageCount` 按实际起止消息重新计算；正文、起止消息和
  摘要内容不变。

从最新真实插件数据重新建立的干净副本演练结果为 `ready`：19 个 Conversation、
251 条 message 和 17 个 Snapshot 全部进入 V2，526 条旧外部关系完成兼容归档，
owner proof 和最终 validation 均无 finding。迁移前后 V1 的 58 个文件数量和整树
SHA-256 完全一致；第二次执行返回 `already-active`。此处仍是副本演练，尚未修改
正式 Vault。
