...
History V2 与对应 Storage Inventory 扫描。全量 V1→V2 reader cutover、跨 Store
retention、Raw GC 和真实迁移仍未完成。
...
- 下一批次：Raw owner graph/GC preview、30/90 天 retention、Archive Catalog、
...
| Phase 2：数据治理 | 进行中 | `b2db2f5` 至 `3b1ca7f` 的治理 checkpoint；live clear/delete、跨 Store authority 与 reference-only History V2 已提交 | Raw GC preview、retention、Archive Catalog、migration validator 与 V2→V1 exporter |
...
1. 提交 live clear/delete coordinator、产品入口、Native authority 与
   Storage Inventory 关联 checkpoint。
2. 接入 History 引用投影、Raw owner graph/GC preview、30/90 天 retention、
   Archive Catalog 产品文案、migration validator 与 V2 → V1 exporter。
...
本批没有执行真实 V1→V2 migration、retention、Raw GC、历史删除、Native cleanup、
部署或 Vault 修改。Phase 2 仍需完成 Raw GC preview、跨 Store 30/90 天 retention、
Archive Catalog、migration validator 与 V2→V1 exporter。
