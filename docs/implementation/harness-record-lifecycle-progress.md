...
Conversation clear/delete 路径；全量 V1→V2 reader cutover、retention、Raw GC
和迁移仍未完成。
...
- 当前开发批次：live clear/delete coordinator、产品入口、Native retirement
  authority 与 Storage Inventory 关联
- 下一批次：Knowledge History 引用投影、Raw owner graph/GC preview、retention
  执行与迁移验证/反向导出
...
| Phase 2：数据治理 | 进行中 | `b2db2f5` 至 `63ce8b0` 的治理 checkpoint；当前 worktree 已接通 live clear/delete 与跨 Store authority | History 投影、Raw GC preview、retention、migration validator 与 V2→V1 exporter |
...
## Live clear/delete coordinator（当前开发批次）
...
checkpoint commit 仍待收口。本批没有部署或修改真实 Vault，也没有执行
