...
GC preview。全量 V1→V2 reader cutover、跨 Store retention、Raw quarantine/
retirement 和真实迁移仍未完成。
...
- 下一批次：30/90 天 retention、Raw quarantine/二次稳定扫描、Archive Catalog、
...
| Phase 2：数据治理 | 进行中 | `b2db2f5` 至 `f1b4caf` 的治理 checkpoint；live clear/delete、History V2 与 metadata-only Raw GC preview 已提交 | retention、Raw quarantine/二次扫描、Archive Catalog、migration validator 与 V2→V1 exporter |
...
部署或 Vault 修改。Phase 2 仍需完成跨 Store 30/90 天 retention、Raw quarantine/
二次稳定扫描、Archive Catalog、migration validator 与 V2→V1 exporter。
