...
恢复证据和可重放 Journal。本轮继续补齐 immutable payload generation 的可恢复
Trash 物理退休、严格 retirement receipt、Journal Storage Inventory、生产
recovery-evidence authority 与启动恢复；自动创建新 retention sweep 仍未接入
维护调度。全量 V1→V2 reader cutover、Raw quarantine/retirement 和真实迁移仍未
完成。
...
| Phase 2：数据治理 | 进行中 | `b2db2f5` 至本轮 physical-retirement checkpoint；live clear/delete、History V2、metadata-only Raw GC preview、Run payload 可恢复物理退休与既有 sweep 启动恢复 | 自动 sweep 调度、Raw quarantine/二次扫描、migration validator 与 V2→V1 exporter |
...
- Retention Journal 已进入 Storage Inventory V2；启动恢复只枚举已原子发布
...
- 自动维护调度仍不得创建新的 retention sweep。该授权要在 Raw quarantine、
