...
- 每个破坏性 Journal 必须在第一个 stage 前发布独立、不可变的 execution plan。
  ...
  只保存 source/trash root ID 和规范化相对路径，Memory/Artifact participant
  保存正式 subject ID 以及 Memory 状态或 Artifact kind，不保存本机绝对路径。
...
- `mark-source-deleted` adapter 必须与 intent 中的 Memory/Artifact participant
  完整同序。Adapter 只先取得各自 Store mutation lane；随后验证 frozen Root
  Binding、恢复既有 Store、再次验证 Root，最后才能 inspect/effect。正式 Store
  缺失时不得靠初始化空 Store 继续。
...
- Workflow Artifact 使用独立 append-only lifecycle chain。首 revision 冻结
  ...
  fail closed。
- Roll-forward 固定先标 Memory/Artifact，再退休 Trash；compensate 先补齐已落盘
  但 Journal 丢失的 forward proof，再恢复 Trash，最后恢复 Memory/Artifact。
