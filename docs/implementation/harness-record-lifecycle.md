...
| Hermes | ACP 公开能力时为 provider-persistent session | runtime 同时公开 list/load 时 native，否则 context rehydrate | ACP 未公开稳定 delete，保持 `unsupported` | 按每次连接的 capability snapshot 决定，不按 backend 名称猜测 |
...
| pending confirmation / unresolved | 不静默保留或删除 | 本轮保留并标记 `sourceDeleted`；丢弃必须先在 Memory 管理中单独完成 | 阻断 Conversation mutation |
...
live Conversation 清空/删除不代理 Memory 或正式 Artifact 的 discard：处置预览只
允许保留并通过各自权威 Store 标记来源变化。用户要丢弃时，必须先在对应的 Memory
或产物管理中完成独立操作，再重新运行 Conversation preview。
...
- policy tombstone 只证明逻辑过期，不能授权 payload 字节消失。物理退休必须经过
  可恢复 Trash，并发布绑定 prior generation、payload SHA、Root Binding、
  prepare/finalization receipt 和 retention transaction 的独立 retirement receipt。
- payload action 固定经过
  `trash-prepared → tombstone-published → source-retired`；Attempt summary 只有在
  payload 已物理退休或明确 not-captured/user-deleted 后才能过期。
- 启动只恢复已发布 plan 的既有 retention transaction，不自动 preview、生成
  transactionId 或创建新 sweep。恢复位于全部本地 commit recovery 之后、Native
  retirement promotion 和 provider cleanup 之前。
- 生产 recovery-evidence authority 必须真实读取维护 WAL、未收口
  RecordMutation、Artifact lineage 与 Run Store，并在完整 retention 回调期间冻结
  这些 writer；空 holds、静态 holds 或检查后不再持锁的 snapshot 不构成执行授权。
...
- Retention Journal 必须进入 metadata-only Storage Inventory。Inventory 可以报告
  transaction/phase/evidence 数量和 corrupt/unsafe/staging finding，但不得输出
  transaction ID、subject ID、相对路径或 Root Binding 的物理位置。
...
- Reverse export 与 reader activation 是两个独立事务。Exporter 只从 manifest
  当前选择的 canonical V2 Store 生成新的、不可变的
  `conversation-v1-exports/export-<digest>/store` 兼容代际；原始 V1、active V2
  和现有 reader route 在导出与验证期间保持不变。
- Export plan 必须绑定 active manifest digest、V2 source fingerprint、投影和
  Store 方向。V1 restore writer 每次 effect 前重读同代 `plan.json`；plan
  缺失、篡改、硬链接、路径逃逸、source 漂移或同 ID 内容冲突全部 fail closed，
  且不得覆盖已有目标记录。
- 导出成功只证明 V1 compatibility generation 可重建相同 portable Conversation
  ledger，不授权 reader 切换。反向激活还需独立 manifest、activation fence、
  精确 namespaced V1 root 和重启恢复；pending fence 期间继续读取 V2。
- 反向 manifest 固定按
  `reverse-copying → reverse-validated → reverse-active` 推进，并同时绑定 forward
  active manifest digest、source/target fingerprint、export generation 和 plan
  digest。active fence 先于终态 entry 耐久发布；fence 后崩溃时 reader 继续读 V2，
  恢复补齐终态 entry 后才切到精确 `v1-export:<generation>`。forward authority、
  plan、manifest chain 或 export 目录祖先缺失/损坏/不安全时必须 fail closed，
  不能回落到保留的 legacy `<storage>/conversations`。
...
- Hermes：本机 0.18.0 已实证公开 session list/load，但未公开稳定 delete。
  - 同时公开 list/load：按精确 ID 使用稳定 `session/load`，加载旧历史不投影成本轮输出。
  - session 消失或 capability 缺失：在 Prompt 前停止或保持 context-rehydrated mode。
  - delete 未公开：cleanup 保持 unsupported，不调用 Hermes 内部数据库方法。
