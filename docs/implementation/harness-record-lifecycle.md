...
5. `Long-term Memory`：经确认和来源追踪的长期事实。
...
flowchart LR
    ...
    C --> M["Long-term Memory<br/>确认后的长期事实"]
    F --> M
    W -. "来源引用" .-> M
    N -. "可丢弃、可重建" .-> C
```
...
| Long-term Memory | 经确认的事实、决策、约束与来源 | 独立治理 | 必须单独选择 |
...
| 清空会话记录 | 保留会话壳，删除消息、snapshot、rolling summary，新建空 context | 关联 payload 按策略删除；共享 Raw 保留；已确认 Memory/正式 Artifact 单独选择 | 异步清理，失败不回滚本地删除 |
| 删除会话 | 删除会话壳与索引 | 同上；保留的 Memory/Artifact 标 `sourceDeleted` | 异步清理，失败进入 quarantine |

所有破坏性操作都走 `RecordMutationJournal`：

```text
planned → staged → committed → cleanup-pending → done / failed
```
...
- 真实 Vault dry-run 报告；不读取 Raw 正文，只做 `lstat`、路径和 hash 元数据检查。
...
metadata-only dry-run；相同输入产生相同 fingerprint，运行前后插件数据目录的
文件数、大小、mtime 与内容指纹不变。
...
5. 启动恢复统一扫描 pending retirement/cleanup。
