...
export interface NativeExecutionStoreSchemaMigrationResult {
...
export class NativeExecutionStore {
  ...
  Promise<NativeExecutionStoreSchemaMigrationResult> {
    return await this.enqueueMutation(async () => {
      const before = await this.readMigrationSnapshotOnce();
      if (before.schemaVersion === NATIVE_EXECUTION_STORE_SCHEMA_VERSION) {
        return {
          status: "already-v2",
      ...
      if (index.version !== 1) {
        throw new NativeExecutionStoreRecoveryRequiredError(
          "schema migration source changed after inventory"
        );
      }
      ...
      const afterIndex = await this.readIndexUnlocked();
      const after = await this.readMigrationSnapshotOnce();
      if (
        after.schemaVersion !== NATIVE_EXECUTION_STORE_SCHEMA_VERSION
        || afterIndex.updatedAt !== index.updatedAt
        || !isDeepStrictEqual(after.records, before.records)
      ) {
        throw new NativeExecutionStoreRecoveryRequiredError(
          "schema migration durable readback does not preserve the V1 authority"
        );
      }
      return {
        status: "migrated",
