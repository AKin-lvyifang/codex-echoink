...
} from "../conversation/conversation-content-projection";
import {
...
} from "./legacy-workspace-migration-identity";
...
export function projectStoredSessionToConversationCommitV2(
  ...
  const workspaceFingerprint = (
    ...
    : legacyWorkspaceMigrationPlaceholderFingerprint(session.cwd);
...
export function projectStoredSessionToLiveConversationCommitV2(
  ...
  input: {
    ...
    replacePayloadForRecordMutation?: "record-clear" | "history-restore";
  }
): ConversationCommitV2 {
  assertExactLegacyKeys(session, LEGACY_SESSION_KEYS, "StoredSession");
  const currentContextId = normalizedIdentifier(session.contextId);
  ...
  const replacementMode = input.replacePayloadForRecordMutation;
  const previousPayloadMessages = input.previous?.payload.messages ?? [];
  const previousMessages = replacementMode
    ? []
    : previousPayloadMessages;
  const previousMessageIds = new Set(
    previousMessages.map((message) => message.id)
  );
  const seenSessionMessageIds = new Set<string>();
  const sessionMessageIndexById = new Map<string, number>();
  const previousIdsInLiveOrder: string[] = [];
  for (const [index, message] of session.messages.entries()) {
    if (seenSessionMessageIds.has(message.id)) {
      throw new Error(
        `Conversation live V2 contains duplicate message ${message.id}`
      );
    }
    seenSessionMessageIds.add(message.id);
    sessionMessageIndexById.set(message.id, index);
    if (previousMessageIds.has(message.id)) {
      previousIdsInLiveOrder.push(message.id);
    }
  }
  if (!replacementMode && !isDeepStrictEqual(
    previousIdsInLiveOrder,
    previousMessages.map((message) => message.id)
  )) {
    throw new Error(
      "Conversation live V2 candidate is missing or reorders durable messages"
    );
  }
  const durableMessages = session.messages.filter((message) =>
    previousMessageIds.has(message.id)
    || isDurableLiveConversationMessage(message)
  );
  if (!replacementMode && !isDeepStrictEqual(
    durableMessages.slice(0, previousMessages.length).map((message) =>
      message.id),
    previousMessages.map((message) => message.id)
  )) {
    throw new Error(
      "Conversation live V2 cannot insert a new durable message before the durable prefix ends"
    );
  }
  const boundary = session.contextStartsAfterMessageId?.trim() || null;
  const boundaryIndex = boundary === null
    ? -1
    : durableMessages.findIndex((message) => message.id === boundary);
  const boundaryLiveIndex = boundary === null
    ? -1
    : sessionMessageIndexById.get(boundary) ?? -1;
  if (boundary !== null && boundaryIndex < 0) {
    throw new Error(
  ...
  if (
    replacementMode === "history-restore"
    && (
      durableMessages.length !== session.messages.length
      || boundary !== durableMessages.at(-1)?.id
    )
  ) {
    throw new Error(
      "Conversation live V2 history restore requires a fully durable payload and a terminal Context boundary"
    );
  }
  const newDurableMessages = durableMessages.filter(
    (message) => !previousMessageIds.has(message.id)
  );
  const messages = replacementMode === "history-restore"
    ? projectHistoryRestoreMessages(
      durableMessages,
      previousPayloadMessages,
      currentContextId
    )
    : [
      ...previousMessages,
      ...newDurableMessages.map((message) => {
      const projected = projectStoredChatMessageToConversationMessageV2(
        message,
        currentContextId
      );
      const liveIndex = sessionMessageIndexById.get(message.id) ?? -1;
      if (boundaryLiveIndex >= 0 && liveIndex <= boundaryLiveIndex) {
        throw new Error(
          "Conversation live V2 cannot insert a new message before the Context boundary"
        );
      }
      return projected;
      })
  ...
  const previousContext = previousMetadata?.currentContext ?? null;
  const reusablePreviousContext = (
    previousContext
    && previousContext.id === currentContextId
    && previousContext.generation === generation
    && previousContext.cwd === session.cwd
    && previousContext.workspaceFingerprint === workspaceFingerprint
    && (previousContext.commitId ?? previousMetadata?.commitId)
      === contextCommitId
  ) ? previousContext : null;
  const currentContext = reusablePreviousContext
    ? reusablePreviousContext.commitId
      ? reusablePreviousContext
      : {
        ...reusablePreviousContext,
        // Pre-V2-cutover Contexts used the containing metadata commit as an
        // implicit identity. Materialize that fallback once so later message
        // appends keep one stable product Context commit.
...
/**
 * Resolves the Store CAS baseline for a live session without mistaking
 * transient UI cards for durable Conversation changes.
 *
 * Existing durable IDs always remain part of the comparison, even if their
 * live presentation was accidentally changed to a transient status. Only
 * additional live messages covered by the V2 non-durable projection policy
 * are ignored. Any other divergence fails closed before Context rotation.
 */
export function durableConversationContentRevisionForLiveSession(
  live: StoredSession,
  durable: StoredSession
): string {
  if (live.id !== durable.id) {
    throw new Error(
      "Conversation recovery required: live and durable Conversation identities differ"
    );
  }
  const durableMessageIds = new Set(
    durable.messages.map((message) => message.id)
  );
  const projectedLive: StoredSession = {
    ...live,
    messages: live.messages.filter((message) =>
      durableMessageIds.has(message.id)
      || isDurableLiveConversationMessage(message)
    )
  };
  const projectedRevision = createConversationContentRevision(projectedLive);
  const durableRevision = createConversationContentRevision(durable);
  if (projectedRevision !== durableRevision) {
    throw new Error(
      `Conversation recovery required: live Conversation ${live.id} diverges from its durable baseline`
    );
  }
  return durableRevision;
}

function projectHistoryRestoreMessages(
  messages: readonly ChatMessage[],
  previousMessages: readonly MessageV2[],
  ...
  const restoredContextId = `history-restored-${createHash("sha256")
  ...
  return messages.map((message) => {
    ...
    if (!previous) {
      return projectStoredChatMessageToConversationMessageV2(
        message,
        restoredContextId
      );
    }
    if (
      ...
      || createConversationProductMessageRevision(previous.message)
        !== createConversationProductMessageRevision(message)
    ) {
      throw new Error(
        "Conversation live V2 history restore reorders or rewrites durable messages"
      );
    }
...
function currentContextBoundaryMessageId(
  ...
  if (firstCurrentIndex < 0) {
    return commit.payload.messages.at(-1)?.id ?? null;
  }
...
function isDurableLiveConversationMessage(message: ChatMessage): boolean {
