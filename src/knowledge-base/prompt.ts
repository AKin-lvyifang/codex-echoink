...
export function buildKnowledgeBasePrompt(input: KnowledgeBasePromptInput): string {
  ...
  return [
    ...
    "",
    "## 标准执行步骤",
    "1. 增量检测：读取 raw/index.md；用 find 列出 raw/ 下所有 .md 文件及修改时间，按修改时间倒序；读取 outputs/.ingest-tracker.md 后对比新增/变更。跳过 raw/ 中以 .base 结尾的辅助文件。",
    "2. Ingest 消化：只处理新增/变更资料；从 wiki/index.md 中选择最匹配领域；写入 wiki/<领域>/ 的 concepts/、guides/、references/ 等合适子文件夹。",
    "3. Wiki 笔记格式：包含 frontmatter（created/updated/tags）、raw 原文回链、3-5 句核心要点、关键概念和术语、与既有 wiki 概念的双向链接。",
    "4. 索引更新：新增或更新 wiki 页面后，同步更新 wiki/index.md、对应领域 00-索引.md 和 raw/index.md。",
    "5. Lint 体检：扫描 wiki/ 下 [[链接]]，检查断链、孤儿页面、过时或 draft 内容，以及 wiki/index.md 链接有效性。",
    "6. 报告输出：如果没有任何变化，报告写“无变化”；如果无法判断归属领域，跳过并在报告中说明。",
    "",
    "## 本轮 raw 来源",
    sourceLines,
    "",
    ...
    "- 禁止修改 raw/ 中的原始资料正文。",
    "- 禁止删除、移动或合并 raw/ 中的文件。",
    "- 禁止自动归档 raw/；如果发现 raw/ 中有应归档内容，只写进报告等待用户确认。",
