...
export function buildKnowledgeBaseRulesTemplate(now: Date): string {
  ...
  return [
    ...
    "## 适用范围",
    "",
    "- 当用户运行 `/check`、`/maintain`、`/reingest`、`/calibrate raw`、`/outputs`、`/inbox`、自动维护、初始化修复等知识库管理动作时，必须按本文的知识库管理边界执行。",
    "- `/check` 只体检，不提炼，不写 Raw 托管属性，不写 Wiki / Projects 正文。",
    "- `/maintain` 固定执行四步提炼协议：读懂原文 -> 拆出知识 -> 融入 Wiki / Projects -> 回写 Raw 已提炼状态。",
    "- `/reingest` 强制重新执行四步提炼；旧摘要型产物如果没有结构化知识和来源证据，不能算完成。",
    ...
    "## 四步提炼协议",
    "",
    "提炼不等于摘要。运行 `/maintain` 或 `/reingest` 并发现 `raw/` 中的新资料时：",
    "",
    "1. 读懂原文：识别主题、来源类型、适用范围和不确定点。",
    "2. 拆出知识：事实、观点、方法、案例、风险、行动项、问题。",
    "3. 融入 Wiki / Projects：优先更新已有主题页；没有稳定承载页才新建。",
    "4. 写来源证据：每个 Raw 都要在 `wiki/` 或 `projects/` 非索引正文页留下来源链接和附近实质知识内容。",
    "5. 回写状态：只有插件验证 Wiki / Projects 正文证据后，才允许写 Raw 托管元属性和 `outputs/.ingest-tracker.md`。",
    "",
    "报告不能代替 Wiki / Projects 正文；`outputs/maintenance` 只能作为过程记录。只写摘要、只写报告、只加来源链接、只更新索引或 tracker，都不能算已提炼。",
