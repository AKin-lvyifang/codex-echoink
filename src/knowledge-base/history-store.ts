...
async function requiresExplicitLegacyCutover(
  ...
  const legacyEntriesExist = await historyDirectoryHasEntries(
    legacySessionsRoot
  );
  if (!legacyIndexExists) return legacyEntriesExist;

  const index = await readLegacyKnowledgeBaseHistoryIndex(
    vaultPath,
    pluginDir
  );
  return index.sessions.length > 0 || legacyEntriesExist;
}
...
async function historyDirectoryHasEntries(root: string): Promise<boolean> {
  return (await readdir(root).catch((error) => {
    if (isNotFoundError(error)) return [];
    throw error;
  })).length > 0;
}
