...
export async function appendScheduledMaintenanceMessage(
  ...
  await settleScheduledMaintenanceLifecycle(
    plugin,
    result,
    afterMessageSaved
  );
...
/**
 * Settles scheduler-owned Native execution and retention state without
 * appending background activity to the interactive Conversation.
 */
export async function settleScheduledMaintenanceLifecycle(
  plugin: CodexForObsidianPlugin,
  result: KnowledgeBaseRunResult,
  afterTerminalSettled: () =>
    Promise<KnowledgeBaseNativeLifecycleSummary | null | void>
): Promise<void> {
  const settings = plugin.settings;
  let lifecycleSummary: KnowledgeBaseNativeLifecycleSummary | null | void;
  try {
    lifecycleSummary = await afterTerminalSettled();
  } catch (error) {
    lifecycleSummary = {
...
function ensureKnowledgeBaseNativeLifecycleRecoveryProjection(
  ...
  const rebaseCleanupReceipt = receipt.issue === "native-cleanup"
  ...
  const changed = settings.lastError !== lastErrorWithRecovery
    ...
    || projectionChanged
    || rebaseCleanupReceipt;
  if (!changed) return false;

  const firstObservedAt = rebaseCleanupReceipt
    ? settings.lastRunAt
  ...
  settings.nativeLifecycleRecoveryReceipt = {
    ...receipt,
    firstObservedAt,
    updatedAt: Math.max(observedAt, firstObservedAt),
    projection: {
      lastErrorBefore,
      lastSummaryBefore,
      lastErrorWithRecovery,
