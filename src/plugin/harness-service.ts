...
export class EchoInkHarnessService {
  ...
  async migrateNativeExecutionStoreSchemaToV2() {
    return await new NativeExecutionStore({
      rootPath: this.nativeExecutionStoreRootPath()
    }).migrateSchemaV1ToV2();
  }
  ...
  async snapshotPendingNativeExecutionRecordIdsForRecovery(input: {
    surface?: string;
    sessionId?: string;
  }): Promise<string[]> {
    const manager = this.getNativeExecutionManager();
    return (await manager.snapshotPendingLocalCommits({
  ...
  async failPendingNativeExecutionsForRecovery(input: {
    reason: string;
    surface?: string;
    sessionId?: string;
    ...
    const recordIds = input.recordIds === undefined
      ? null
      : new Set(input.recordIds.map((recordId) => recordId.trim()).filter(Boolean));
    ...
    const pending = (await manager.listPendingLocalCommits(filter)).filter(
      (record) =>
        ...
        && (recordIds === null || recordIds.has(record.id))
    );
