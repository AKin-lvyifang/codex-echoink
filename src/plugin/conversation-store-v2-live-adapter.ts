...
import {
  ...
  type ConversationStoreV2CommitOptions
...
export class FileConversationStoreV2LiveAdapter {
  ...
  async readSession(sessionId: string): Promise<StoredSession | null> {
    ...
    return commit
      ? projectConversationCommitV2ToLiveStoredSession(commit)
      : null;
  }
  ...
  async commitSessionContext(
    ...
    const currentContextCommitId =
      current.metadata.currentContext.commitId ?? current.metadata.commitId;
    const targetContextId = session.contextId?.trim();
    const targetContextCommitId = session.commitId?.trim();
    if (!targetContextId || !targetContextCommitId) {
      throw new ConversationContextConflictError(
        `Conversation ${session.id} target Context identity is incomplete`
      );
    }
    const committed = await this.commitCandidate(session, current, {
      contextCommit: {
        sourceGeneration: current.metadata.currentContext.generation,
        sourceContextId: current.metadata.currentContext.id,
        sourceContextCommitId: currentContextCommitId,
        targetGeneration: sessionGeneration(session),
        targetContextId,
        targetContextCommitId
      }
    });
  ...
  async commitConversationRecordClear(
    ...
    const candidate = projectStoredSessionToLiveConversationCommitV2(
      session,
      {
        ...
        replacePayloadForRecordMutation: "record-clear"
      }
    );
  ...
  async commitConversationHistoryRestore(
    ...
    if (!mutationId) {
      throw new ConversationContextConflictError(
        "Conversation V2 history restore requires its mutationId"
      );
    }
    const current = await this.requireExpectedSession(session.id, options);
    const candidate = projectStoredSessionToLiveConversationCommitV2(
      session,
      {
        previous: current,
        metadataCommitId: liveMetadataCommitId(),
        replacePayloadForRecordMutation: "history-restore"
      }
    );
    const committed = await this.store.commitConversation(candidate, {
      expectedRevision: current.metadata.revision,
  ...
  async persistSettingsSessions(
    ...
    for (const session of settings.sessions) {
      ...
      const committed = await this.commitCandidate(session, current);
      options.afterSessionPersisted?.(
        projectConversationCommitV2ToLiveStoredSession(committed)
      );
  ...
  async createPristineSession(session: StoredSession): Promise<void> {
    ...
    const current = await this.store.readConversation(session.id);
    if (current) {
      const durable = projectConversationCommitV2ToLiveStoredSession(current);
      if (
    ...
    const candidate = isWorkspaceFreePristineChat(session)
  ...
  >, options: Pick<
    ConversationStoreV2CommitOptions,
    "contextCommit"
  > = {}): Promise<ConversationCommitV2> {
    const pristine = current && workspaceFreePristineChatFromCommit(current);
    if (
      current
      && pristine
      && isWorkspaceFreePristineChat(session)
      && createConversationContentRevision(pristine)
        === createConversationContentRevision(session)
    ) {
      return current;
    }
...
function projectConversationCommitV2ToLiveStoredSession(
  commit: ConversationCommitV2
): StoredSession {
  return workspaceFreePristineChatFromCommit(commit)
    ?? projectConversationCommitV2ToStoredSessionV1(commit);
}

// V2 requires a complete currentContext envelope even before a Chat has a
// workspace. Only the exact deterministic generation-1 envelope is decoded
// back to the workspace-free product state; any drift remains a hard conflict.
function workspaceFreePristineChatFromCommit(
  commit: ConversationCommitV2
): StoredSession | null {
  ...
  return isDeepStrictEqual(
    commit,
    projectStoredSessionToConversationCommitV2(candidate)
  )
    ? candidate
    : null;
}

function isWorkspaceFreePristineChat(session: StoredSession): boolean {
  return Boolean(
    session.id.trim()
    ...
    && Object.keys(session.backendBindings ?? {}).length === 0
