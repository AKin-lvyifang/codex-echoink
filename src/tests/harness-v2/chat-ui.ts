...
export async function runHarnessV3ChatUiTests(): Promise<void> {
  ...
  assert.match(messageListSource, /codex-kb-run-phase-rail/);
  assert.match(messageListSource, /codex-kb-run-phase-body/);
  assert.match(messageListSource, /codex-kb-run-phase-status-copy/);
  assert.match(messageListSource, /codex-kb-maintain-section-chevron/);
  assert.match(messageListSource, /openKnowledgeBaseReportSections\.set\(sectionStateKey, details\.open\)/);
  assert.match(messageListSource, /codex-kb-maintain-detail-link/);
  assert.doesNotMatch(messageListSource, /createSpan\(\{\s*cls:\s*`codex-kb-run-cell/);
  ...
  assert.match(styles, /\.codex-kb-run-card\s*\{[^{}]*border:\s*0[^{}]*background:\s*transparent[^{}]*box-shadow:\s*none/);
  assert.match(styles, /\.codex-kb-run-track\s*\{[^{}]*display:\s*grid/);
  assert.match(styles, /\.codex-kb-run-node\s*\{[^{}]*grid-template-columns:\s*18px\s+minmax\(0,\s*1fr\)[^{}]*min-height:\s*28px/);
  assert.match(styles, /\.codex-kb-run-node-icon\s*\{[^{}]*width:\s*18px[^{}]*height:\s*18px[^{}]*border-radius:\s*50%/);
  assert.match(styles, /\.codex-kb-maintain-card\s*\{[^{}]*border:\s*0[^{}]*background:\s*transparent[^{}]*box-shadow:\s*none/);
  assert.match(styles, /\.codex-kb-maintain-icon\s*\{[^{}]*width:\s*22px[^{}]*height:\s*22px[^{}]*border-radius:\s*50%/);
  assert.match(styles, /\.codex-kb-maintain-section\s*\{[^{}]*border:\s*0[^{}]*border-top:\s*1px\s+solid/);
  assert.match(styles, /\.codex-kb-maintain-section-count\s*\{[^{}]*background:\s*transparent/);
  assert.match(styles, /\.codex-kb-maintain-detail-link\s*\{[^{}]*cursor:\s*pointer/);
