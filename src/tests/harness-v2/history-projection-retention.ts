...
export async function runHarnessV2HistoryProjectionRetentionTests(): Promise<void> {
  ...
  await assertUniqueLegacySideCopyDoesNotBlockOrdinaryPersistence();
  ...
  assertCleanupReceiptRebasesWhenProjectedOntoNewRun();
...
async function assertUniqueLegacySideCopyDoesNotBlockOrdinaryPersistence():
Promise<void> {
  const vaultPath = await mkdtemp(
    path.join(tmpdir(), "echoink-history-v1-side-copy-")
  );
  const date = "2026-01-20";
  const message = messageForDate("history-v1-authoritative", date);
  const session = knowledgeSession(
    vaultPath,
    "knowledge-history-v1-side-copy",
    [message]
  );
  const store = new FileConversationStore({
    rootPath: path.join(
  ...
  try {
    ...
    const sideCopyPath = path.join(
      path.dirname(legacy.dayPath),
      `${date} 2.jsonl`
    );
    const sideCopyBytes = `${JSON.stringify({
      ...message,
      id: "history-v1-unindexed-side-copy",
      text: "preserved but not authoritative"
    })}\n`;
    await writeFile(sideCopyPath, sideCopyBytes, "utf8");
    await persistAndCompactKnowledgeBaseHistory(
      vaultPath,
      PLUGIN_DIR,
      settingsForKnowledgeSession(session),
    ...
    assert.equal(
      await readKnowledgeBaseHistoryActive(vaultPath, PLUGIN_DIR),
      null,
      "ordinary persistence must retain the explicit V1 cutover gate"
    );
    ...
    assert.equal(
      await readKnowledgeBaseHistoryActive(vaultPath, PLUGIN_DIR),
      null,
      "an unindexed unique file must keep explicit V2 cutover blocked"
    );
    assert.equal(await readFile(sideCopyPath, "utf8"), sideCopyBytes);
    await assertLegacyFixtureBytesUnchanged(legacy);
  } finally {
    await rm(vaultPath, { recursive: true, force: true });
  }
...
async function assertSettingsSaveReprojectsDurableRecoveryReceipt(): Promise<void> {
  ...
  try {
    ...
    await store.withExclusiveTransaction(async (transaction) => {
      ...
      baseline.settings.lastRunAt = 2_000;
    ...
    assert.equal(
      settings.knowledgeBase.nativeLifecycleRecoveryReceipt?.firstObservedAt,
      2_000,
      "a cleanup receipt projected onto a newer result must be timestamped for that result"
    );
    assert.equal(
      settings.knowledgeBase.nativeLifecycleRecoveryReceipt?.updatedAt,
      2_000
    );
    ...
    assert.equal(
      persisted.nativeLifecycleRecoveryReceipt?.firstObservedAt,
      2_000
    );
...
function assertCleanupReceiptRebasesWhenProjectedOntoNewRun(): void {
  const settings = normalizeSettingsData({
    settingsVersion: 29,
    knowledgeBase: {
      lastRunAt: 100,
  ...
  settings.lastRunAt = 200;
  settings.lastError = "";
  settings.lastSummary = "批次 B 新维护摘要";
  settings.lastWarnings = [];
  const rebased = reconcileKnowledgeBaseNativeLifecycleRecovery(
    settings,
    null,
    250
  );

  assert.equal(rebased.state, "recovery-pending");
  assert.equal(settings.nativeLifecycleRecoveryReceipt?.firstObservedAt, 200);
  assert.equal(settings.nativeLifecycleRecoveryReceipt?.updatedAt, 250);
  assert.equal(
    settings.nativeLifecycleRecoveryReceipt?.projection.lastSummaryBefore,
    "批次 B 新维护摘要"
  );
  assert.match(settings.lastSummary, /批次 B 新维护摘要.*清理待恢复/);
}
