...
import { CodexMessageListRenderer, knowledgeBaseMaintainExecutionItems, knowledgeBaseMaintainReportItemPath, knowledgeBaseMaintainSectionOpenState, knowledgeBaseRunEventCopy, knowledgeBaseRunProgressState, knowledgeBaseRunProgressStateFromEvents, messageListVirtualHeight, scrollTopForMessageListBottom, shouldPinMessageListBottom } from "../ui/codex-view/message-list";
...
import { runHarnessV2ConversationStoreRouterTests } from "./harness-v2/conversation-store-router";
...
import { runHarnessV2RecordMutationExecutionRuntimeTests } from "./harness-v2/record-mutation-execution-runtime";
import { runHarnessV2RecordMutationProductionTests } from "./harness-v2/record-mutation-production";
...
import { runHarnessV2RecordMutationSourceBundleTests } from "./harness-v2/record-mutation-source-bundle";
import { runHarnessV2RecordMutationSourceParticipantTests } from "./harness-v2/record-mutation-source-participant";
...
import { KNOWLEDGE_BASE_COMMAND_GUIDE, getTrailingSlashQuery, knowledgeBaseHelpText, knowledgeCommandOptions, knowledgeCommandQueryForInput, parseKnowledgeBaseCommand, requiresMaintenanceAuthority, shouldHandleKnowledgeBaseCommand } from "../knowledge-base/commands";
...
const kbRecoveryAskTurnOptions = buildCodexKnowledgeTurnOptions({
  settings: { ...DEFAULT_SETTINGS, mcpEnabled: true },
  availableModels: [{ model: "gpt-test" }],
  vaultPath: "/vault",
...
assert.equal(kbRecoveryAskTurnOptions.permission, "read-only");
assert.equal(kbRecoveryAskTurnOptions.mcpEnabled, false);
assert.equal(kbRecoveryAskTurnOptions.externalResources, "disabled");
assert.deepEqual(kbRecoveryAskTurnOptions.workspaceResources, {
  plugins: {},
  mcpServers: {},
  skills: {}
});
...
assert.equal(DEFAULT_SETTINGS.settingsVersion, 40);
...
try {
  ...
  const scheduler = new KnowledgeBaseScheduler({
    ...
    settleScheduledMaintenance: async (result) => {
...
assert.equal(knowledgeBaseMaintainSectionOpenState(undefined, true), true);
assert.equal(knowledgeBaseMaintainSectionOpenState(false, true), false, "an explicitly collapsed report section must stay collapsed after a measured-row rerender");
assert.equal(knowledgeBaseMaintainSectionOpenState(true, false), true, "an explicitly expanded report section must stay expanded after a measured-row rerender");
assert.equal(knowledgeBaseMaintainReportItemPath({ title: "raw/articles/legacy-note.md", description: "legacy payload" }), "raw/articles/legacy-note.md");
assert.equal(knowledgeBaseMaintainReportItemPath({ title: "raw/articles/current-note.md", path: "raw/articles/current-note.md", description: "current payload" }), "raw/articles/current-note.md");
assert.equal(knowledgeBaseMaintainReportItemPath({ title: "结构风险", description: "not a file" }), undefined);
...
for (const safeIntent of ["chat", "ask", "help", "history", "clear", "cancel"] as const) {
  assert.equal(requiresMaintenanceAuthority(safeIntent), false, `${safeIntent} must not require maintenance write authority`);
}
for (const writeIntent of ["init", "maintain", "lint", "reingest", "calibrate", "process-outputs", "process-inbox", "journal", "review", "collect"] as const) {
  assert.equal(requiresMaintenanceAuthority(writeIntent), true, `${writeIntent} must require maintenance write authority`);
}
...
assert.match(
  knowledgeBaseManagerSource,
  /settleScheduledMaintenanceLifecycle/,
  "scheduled maintenance must settle lifecycle state outside interactive Conversation"
);
assert.doesNotMatch(
  knowledgeBaseManagerSource,
  /appendScheduledMaintenanceMessageToSession/,
  "the production scheduler must not append background activity to interactive Conversation"
);
...
const mainPluginLineBudget = 250;
...
const activateKnowledgeBaseChannelSource = pluginViewServiceSource.slice(
  pluginViewServiceSource.indexOf("async activateKnowledgeBaseChannel"),
  pluginViewServiceSource.indexOf("applyComposerDefaultsToView")
);
const activateCodexViewSource = pluginViewServiceSource.slice(
  pluginViewServiceSource.indexOf("async activateView"),
  pluginViewServiceSource.indexOf("async activateKnowledgeBaseChannel")
);
const flushSessionSaveSource = codexViewMessageControllerSource.slice(
  codexViewMessageControllerSource.indexOf("async function flushSessionSave"),
  codexViewMessageControllerSource.indexOf("function moveMessageToEnd")
);
assert.ok(
  mainPluginLineCount <= mainPluginLineBudget,
  `src/main.ts should stay under ${mainPluginLineBudget} lines, got ${mainPluginLineCount}`
);
...
assert.match(activateKnowledgeBaseChannelSource, /this\.plugin\.settings\.activeSessionId\s*=\s*session\.id/);
assert.match(
  activateKnowledgeBaseChannelSource,
  /await this\.plugin\.saveSettings\(\s*true,\s*\{\s*flushConversationStore:\s*false,\s*flushKnowledgeBaseHistory:\s*false\s*\}\s*\)/,
  "activating the knowledge channel must persist the active session without recommitting conversation or history"
);
assert.ok(
  activateKnowledgeBaseChannelSource.indexOf("activeSessionId = session.id")
    < activateKnowledgeBaseChannelSource.indexOf("await this.plugin.saveSettings"),
  "the knowledge session selection must be applied before its isolated settings save"
);
assert.match(activateCodexViewSource, /leaves\.find\(\(candidate\) => candidate\.view instanceof CodexView\)/);
assert.match(activateCodexViewSource, /setViewState\(\{ type: "empty", active: false \}\)[\s\S]*setViewState\(\{ type: VIEW_TYPE_CODEX, active: true \}\)/);
assert.match(flushSessionSaveSource, /if \(!host\.sessionSaveTimer\) return;[\s\S]*saveSettings\(true\)/);
...
const homeDashboardFile = (filePath: string, mtime: number, extra: Partial<KnowledgeBaseDashboardFile> = {}): KnowledgeBaseDashboardFile => ({ path: filePath, size: 1, mtime, ...extra });
const trustedHomeFingerprint = "sha256:1:trusted-home";
const homeCards = buildHomeCards({
  raw: {
    recentFiles: [
      ...
      homeDashboardFile("raw/articles/trusted-source.md", 1700000000004, {
        fingerprint: trustedHomeFingerprint,
        rawDigest: {
          processed: true,
          status: "已提炼",
          fingerprint: trustedHomeFingerprint,
          digestedAt: 1,
          reportPath: "outputs/maintenance/trusted.md",
          evidencePaths: ["wiki/trusted.md"]
        }
      })
    ],
    changedCount: 1,
...
assert.deepEqual(homeCards.map((card) => card.path).sort(), [
  ...
  "raw/articles/trusted-source.md",
  "wiki/ai/real-page.md"
]);
assert.equal(homeCards.find((card) => card.path === "raw/articles/real-source.md")?.status, "Raw 待提炼");
assert.equal(homeCards.find((card) => card.path === "raw/articles/trusted-source.md")?.status, "已提炼");
assert.deepEqual(buildHomeRawBatchPreview(homeCards)?.previewPaths, ["raw/articles/real-source.md"]);
...
const recoveryBlockedRunProgress = knowledgeBaseRunProgressStateFromEvents("recovery-blocked", [
  { type: "workflow.phase.started", phaseId: "prepare", status: "running", createdAt: 1 },
  { type: "workflow.phase.failed", phaseId: "prepare", status: "failed", message: "阶段进度见下方", createdAt: 2 }
], maintainRunPayload.phases.length);
assert.equal(recoveryBlockedRunProgress?.filledCells, 0);
assert.equal(recoveryBlockedRunProgress?.activeIndex, 0);
...
assert.deepEqual(maintainReportPayload.sections.find((section) => section.id === "digested")?.items.map((item) => item.path), ["raw/articles/a.md", "raw/articles/b.md"]);
...
const partialMaintainReportPayload = buildKnowledgeBaseMaintainReportPayload("maintain", {
  status: "success",
  completion: "partial",
  reportPath: "outputs/maintenance/kb-maintenance-2026-07-10.md",
  summary: "部分完成。",
  processedSources: [],
  pendingSources: ["raw/articles/podcast-transcript.md"]
});
assert.equal(partialMaintainReportPayload.sections.find((section) => section.id === "pending-sources")?.items[0]?.path, "raw/articles/podcast-transcript.md");
...
assert.deepEqual(customLaunch.args.slice(0, 7), [
  "app-server",
  "--listen",
  "stdio://",
  "--disable",
...
assert.deepEqual(loginLaunch.args, [
  "app-server",
  "--listen",
  "stdio://",
  "--disable",
...
try {
  ...
  try {
    ...
    let scheduledAppendExternalizeCalls = 0;
    ...
    const scheduledAppendManager = new KnowledgeBaseManager({
      ...
      externalizeMessageText: async (message: ChatMessage) => {
        scheduledAppendExternalizeCalls += 1;
  ...
  try {
    ...
    let scheduledSurfaceRefreshShouldFail = false;
    ...
    const scheduledRefreshManager = new KnowledgeBaseManager({
      ...
      externalizeMessageText: async () => undefined,
      getCodexView: () => ({
        refreshAfterBackgroundKnowledgeMessage: () => undefined,
        refreshKnowledgeBaseDashboard: () => {
          if (scheduledSurfaceRefreshShouldFail) {
    ...
    (scheduledRefreshManager as any).runMaintenance = async (
      ...
      scheduledSurfaceRefreshShouldFail = true;
      return durableScheduledSuccessResultForTest(
    ...
    assert.equal(scheduledRefreshSettings.sessions[0].messages.length, 0);
    assert.equal(scheduledRefreshSaveCalls, 2);
    assert.equal(scheduledRefreshWarnings.length, 1);
    assert.equal(scheduledRefreshWarnings[0][0], "每日知识库维护界面刷新失败");
  } finally {
  ...
  try {
    ...
    let scheduledConcurrentExternalizeCalls = 0;
    ...
    const scheduledConcurrentManager = new KnowledgeBaseManager({
      ...
      externalizeMessageText: async () => {
        scheduledConcurrentExternalizeCalls += 1;
    ...
    assert.equal(scheduledConcurrentSettings.knowledgeBase.lastError, "");
    assert.equal(scheduledConcurrentSettings.sessions.some((session) => session.id === "manual-session-created-during-scheduled-message"), false);
    assert.equal(scheduledConcurrentSettings.sessions.find((session) => session.id === "kb-scheduled-concurrent")?.messages.length, 0);
    assert.equal(scheduledConcurrentExternalizeCalls, 0);
  ...
  try {
    ...
    let scheduledSameSessionExternalizeCalls = 0;
    ...
    const scheduledSameSessionManager = new KnowledgeBaseManager({
      ...
      failPendingNativeExecutionsForRecovery: async () => 0,
      externalizeMessageText: async () => {
        scheduledSameSessionExternalizeCalls += 1;
    ...
    assert.notEqual(session.updatedAt, manualMessageCreatedAt);
    assert.equal(scheduledSameSessionExternalizeCalls, 0);
...
try {
  ...
  await (manager as any).waitUntilMaintenanceReady();
  assert.equal(manager.maintenanceRecoveryStatus.state, "ready");
...
try {
  ...
  const trustedLargeBody = `# Trusted large\n\n${"large transcript content\n".repeat(14_000)}`;
  const trustedLargePath = "raw/articles/trusted-large.md";
  const trustedLargeFingerprint = rawDigestFingerprint(trustedLargePath, Buffer.from(trustedLargeBody, "utf8"));
  await writeFile(path.join(dashboardRawDigestStatusVault, trustedLargePath), [
    "---",
    "已处理: true",
    "提炼状态: 已提炼",
    "提炼时间: 2026-06-03T17:44:04.360Z",
    `提炼指纹: ${trustedLargeFingerprint}`,
    "提炼报告: outputs/maintenance/kb-raw-calibration-2026-06-04.md",
    "提炼证据:",
    "  - wiki/trusted-large.md",
    "---",
    trustedLargeBody
  ].join("\n"), "utf8");
    ...
    knowledgeBase: {
      processedSources: {
        ...
        [trustedLargePath]: {
          size: 1,
          mtime: 1,
          fingerprint: trustedLargeFingerprint,
          digestedAt: 1
        }
      }
    }
  ...
  assert.ok(dashboard.recommendations.cards.some((card) => card.path === trustedLargePath && card.status === "已提炼"));
  const dashboardRawBatchPreview = buildHomeRawBatchPreview(buildHomeCards(dashboard));
  assert.ok(dashboardRawBatchPreview?.previewPaths.includes("raw/articles/pending.md"));
  assert.equal(dashboardRawBatchPreview?.previewPaths.includes(trustedLargePath), false);
} finally {
...
await runHarnessV2ConversationStoreRouterTests();
...
await runHarnessV2RecordMutationExecutionRuntimeTests();
await runHarnessV2RecordMutationProductionTests();
...
await runHarnessV2RecordMutationSourceBundleTests();
await runHarnessV2RecordMutationSourceParticipantTests();
