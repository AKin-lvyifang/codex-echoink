...
const ZH_CN = {
  ...
  knowledge: {
    ...
    memoryNote2: "各类插件记录按独立保留期留在本地；详细 Run payload 默认 30 天、Run summary 默认 90 天。Chat 和 /ask 会收到仍可用记录的按需查询目录；index.json 是提炼后的正式结构化真源。.codex-memory 只作为显式迁移来源。",
...
const EN: SettingsCopy = {
  ...
  knowledge: {
    ...
    memoryNote2: "Plugin records stay local under independent retention windows: detailed Run payloads default to 30 days and Run summaries to 90 days. Chat plus /ask receive an on-demand catalog for records that remain available. index.json is the canonical curated source; .codex-memory is an explicit migration source only.",
