...
const ZH_CN = {
  ...
  knowledge: {
    ...
    memoryNote1: "长期记忆已内置到 EchoInk Harness，统一保存在 Vault/.echoink/memory；Codex、OpenCode、Hermes 只是可替换的整理执行引擎。",
    memoryNote2: "index.json 是唯一正式结构化真源，current/spec/tasks/archive Markdown 由它自动投影。.codex-memory 只作为显式迁移来源，导入不会删除源目录。",
    memoryEnabled: "启用长期记忆",
    memoryEnabledDesc: "普通 Chat 和 /ask 会读取并选择性采集；维护类 workflow 只在本地提交成功后记录结果。",
    memoryAutoSync: "自动同步",
...
const EN: SettingsCopy = {
  ...
  knowledge: {
    ...
    memoryHeading: "EchoInk Memory V2",
    memoryNote1: "Long-term memory is built into the EchoInk Harness and stored in Vault/.echoink/memory. Codex, OpenCode, and Hermes are interchangeable curator engines.",
    memoryNote2: "index.json is the only canonical structured source. current/spec/tasks/archive Markdown files are deterministic projections. .codex-memory is available only as an explicit, read-only migration source.",
    memoryEnabled: "Enable long-term memory",
    memoryEnabledDesc: "Chat and /ask retrieve and selectively capture. Maintenance workflows record results only after the local commit succeeds.",
