...
const caseDirectory = async (parent) => {
  const names = await readdir(path.join(vault, parent));
  const matches = names.filter((name) => name === "ask-reading-v1" || name.endsWith("（ask-reading-v1）"));
  if (matches.length > 1) throw new Error(`Ambiguous existing case directory under ${parent}`);
  return matches[0] ?? "ask-reading-v1";
};
const wikiCase = await caseDirectory(wiki), rawCase = await caseDirectory(raw), projectCase = await caseDirectory(projects);
const wp = (name) => `${wiki}/${wikiCase}/${name}.md`;
const rp = `${raw}/${rawCase}/上海大会报销补充通知.md`;
